...
def assignTask(request):
    ...
    if request.method == "GET":
        try:
            ...
            if len(qs) > 0:
                ...
                if qs[0].contact and qs[0].contact in users:
                    return render(request, pageUrl, {**default_values, 'users': users, 'contact': qs[0].contact, 'priority': qs[0].priority, 'class_field': qs[0].class_field, 'task': qs[0].task, 'taskno': taskno})
        ...
        try:
            ...
            priority = None if not request.POST.get("priority") else request.POST.get("priority")
            class_field = None if not request.POST.get("class_field") else request.POST.get("class_field")
            tasknoArr = taskno.split("-")
            qs = models.Task.objects.filter(pid=tasknoArr[0], tid=tasknoArr[1], taskid=tasknoArr[2])
            if len(qs) > 0:
                task = qs[0]
                ...
                users = UserService.GetPartUserNames()
                return render(request, pageUrl, {**default_values, 'message': _("Assign Successful!"), 'users': users, 'contact': contact, 'priority': priority, 'class_field': class_field, 'task': task.task, 'taskno': taskno})
            else:
                users = UserService.GetPartUserNames()
