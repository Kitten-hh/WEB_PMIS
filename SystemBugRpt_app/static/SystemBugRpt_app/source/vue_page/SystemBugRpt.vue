              ...
              <div :class="['form-group row search_list mb-2', lang_code_en ? 'lang_en' : '']" name="search_list">
                ...
                <div class="form-group d-flex align-items-center col-12 col-sm-4 col-lg-6 col-xxl-4 col-xxxxl-3 query_issueType mb_xxxxl_2">
                  ...
                  <select
                    ...
                    :disabled="([0].indexOf(state) > -1)"
                  >
                    <option value="1">1:需求</option>
                    <option value="2">2:維護</option>
                    <option value="3">3:維修</option>
                    <option value="4">4:穩定性</option>
                    <option value="5">5:添加字段</option>
                    <option value="6">6:邏輯增強</option>
                    <option value="7">7:邏輯變更</option>
                  </select>
                </div>
                ...
                <div class="form-group d-flex align-items-center col-12 col-sm-4 col-lg-6 col-xxl-4 col-xxxxl-3 mb_xxxxl_2">
                  ...
                  <select
                    ...
                    :disabled="([0].indexOf(state) > -1)"
                  >
                    <option value="N">N:未處理</option>
                    <option value="I">I:進行中</option>
                    <option value="T">T:處理中</option>
                    <option value="F">F:已處理</option>
                  </select>
                </div>
                <div class="form-group d-flex align-items-center col-12 col-sm-4 col-lg-6 col-xxl-4 col-xxxxl-3 mb_xxxxl_2">
                  <label class="col-form-label col-auto">{{$t("Hoperation")}}</label><!--操作 Hoperation-->
                  <select
                    class="form-control col"
                    v-model="currentMaster.hoperation"
                    :disabled="!([2].indexOf(state) > -1)"
                  >
                    <option value=""></option>
                    <option value="A">A:分配</option>
                    <option value="D">D:延遲</option>
                    <option value="T">E:暫定</option>
                    <option value="F">F:Fixed Day</option>
                    <option value="L">L:不必處理</option>
                    <option value="M">M:討論</option>
                    <option value="O">O:規則(Policy)</option>
                    <option value="P">P:馬上處理</option>
                    <option value="R">R:參考(Reference)</option>
                    <option value="U">U:指令(Instruction)</option>
                    <option value="W">W:等待(Waiting)</option>
                  </select>
                </div>
                <div class="form-group d-flex align-items-center col-12 col-sm-4 col-lg-6 col-xxl-4 col-xxxxl-3 mb_xxxxl_2">
                ...
                <div class="form-group col-12 query_issueDescp">
                  ...
                  <textarea type="text" class="form-control col" :rows="textareaRows" v-model="currentMaster.task" :disabled="!([2].indexOf(state) > -1)"></textarea>
                </div>
              </div>
            ...
            <div class="tab-content pt-3">
              ...
              <!--其它部門-->
                <div :class="['form-group row search_list mb-0', lang_code_en ? 'lang_en' : '']" name="search_list">
                  ...
                  <div class="form-group d-flex align-items-center col-12 col-sm-4 col-lg-6 col-xxl-4 col-xxxl-3" >
                    <label class="col-form-label col-auto">{{$t("ADMRP_rp020")}}</label><!--系統名稱 ADMRP_rp020-->
                    <input
                      type="text"
                      class="form-control col"
                      v-model="currentMaster.rp020c"
                  ...
                  <div class="form-group d-flex align-items-center col-12 col-sm-4 col-lg-6 col-xxl-4 col-xxxl-3" >
                    <label class="col-form-label col-auto">{{$t("ADMRP_rp027")}}</label><!--功能描述 ADMRP_rp027-->
              ...
              <!--電腦部-->
                <div :class="['form-group row search_list mb-1', lang_code_en ? 'lang_en' : '']" name="search_list">
                  ...
                  <div class="form-group d-flex align-items-center col-12 col-sm-6 col-xxl-4" >
                    <label class="col-form-label col-auto">{{$t("Session")}}</label><!-- 模塊 Session-->
                    <textarea type="text" class="form-control col" v-model="currentMaster.sessioninfo" @click="showSessionModal" :disabled="([0].indexOf(state) > -1)" style="flex: 1;"></textarea>
                  </div>
                  <div class="form-group d-flex align-items-center col-12 col-sm-6 col-xxl-4" >
...
<!--選擇項目編號-->
<LPModal ref="projectModal_ref" class="customModal">
  <template v-slot:body>
    <button type="button" class="btn btn-secondary order-button" data-v-8ec5118a="" @click="searchProjectClick">
    ...
    <LPDataTable
      ...
      :custom_params_fun="projectParamsFun"
      :custom_options="projectOptions"
      @on_row_click="project_row_click"
      datasource="/systembugrpt/subproject?format=datatables"
      ref="SubProjectTable"
    />
  </template>
...
<!--選擇Session編號-->
<LPModal ref="sessionModal_ref" class="customModal">
  <template v-slot:body>
    <LPDataTable
      :paging="false"
      :pageLength="100"
      ...
      :custom_options="sessionOptions"
      :custom_params_fun="sessionParamsFun"
      @on_row_click="session_row_click"
      :datasource="[]"
      ref="SessionTable"
    />
  </template>
  <template v-slot:footer>
      <button type="button" class="btn btn-primary"  @click="sessionClick">{{$t('Confirm')}}</button>
      <button type="button" class="btn btn-light" data-dismiss="modal">{{$t('Cancel')}}</button>
  </template>
</LPModal>
<!--選擇Session編號-->
...
data() {
  var self = this
  return {
    sessionColumns:[ //Session列表 V_TaskList
      { field: "sessionid", label: this.$t('ModuleObject_moduleid')}, //Session編號
      { field: "sdesp", label: this.$t('Remark')}, //描述
    ],
    sessionOptions:{
      deferLoading: 0,
      scrollY: 300,
    },
    sessionParamsFun: undefined,
      ...
      { field: "",
        ...
        render: function (data, type, row) {
          ...
          row.item_array.forEach(item => {
              // 格式化 planbdate_item
              const planbdate = item.planbdate_item 
                  ? new Date(item.planbdate_item).toISOString().split('T')[0] 
                  : '';

              // 格式化 planedate_item
              const planedate = item.planedate_item 
                  ? new Date(item.planedate_item).toISOString().split('T')[0] 
                  : '';

              tableHtml += `
                  <tr>
                      <td>${item.contact_item}</td>
                      <td class="text-truncate" title="${item.task_item}" style="max-width:120px">${item.task_item}</td>     
                      <td class="text-truncate" title="${item.remark_item}"  style="max-width:120px">${item.remark_item}</td>
                      <td>${item.progress_item}</td>
                      <td>${planbdate}</td>
                      <td>${planedate}</td>
                  </tr>`;
          });
    ...
    projectColumns:[ //子工程列表 SubProject
      { field: "recordid", label: this.$t('Project ID')}, //項目編號 Project ID
      { field: "projectname", label: this.$t('V_Task_pname')}, //項目名稱
    ],
    projectParamsFun: undefined,
...
mounted() {
  ...
  this.setModelBackdrop('projectModal_ref');
  this.setModelBackdrop('sessionModal_ref');
  ...
  this.$nextTick(() => {
    ...
    this.$refs.AdmrpTable.datatable.one('draw.dt', () => {
        // 畫面重繪完成後執行後續操作
      const tableData = this.$refs.AdmrpTable.datatable.data();
      this.getMasterInfo(tableData[0].inc_id, '');
      this.$refs.Admrp_list.click();
    });
  });
...
methods: {
  ...
  searchProjectClick(){
    var self = this;
    var columns = [];
    for (let item of self.projectColumns) {
        // if (["", "inc_id"].indexOf(item.field) != -1) continue;
        columns.push(item);
    }
    var query = self
    .GetQueryExp(
        "SystemBugRpt_vueFrm_SubProjectTable",
        columns
    )
    query.then((query) => {
      var query_value = {}
      if (query.master != undefined)
        ...
        self.projectParamsFun = function () {
            return query_value;
        };
        self.$nextTick(function () {
            self.$refs.SubProjectTable.datatable.column(0).search("").draw();
        });
    });
  },
  //用戶選擇模塊編號
  sessionClick(){
    if(this.currentTamp.sessioninfo == undefined) return this.showMessage(this.$t('Unselected data'));
    this.currentMaster.sessioninfo = this.currentTamp.sessioninfo;
    this.$refs.sessionModal_ref.hide();
  },
  //Session列表單擊事件 
  session_row_click(event,data){
    this.currentTamp.sessioninfo = data.sessionid + '(' + (data.sdesp || '') + ')';
  },
  //獲取session列表
  get_sessionTable(recordid){
    axios.get(`/systembugrpt/get_vtasklist`,{params:{recordid:recordid}}).then(response =>{
      if(response.data.status){
        this.$refs.SessionTable.datatable.clear().rows.add(response.data.data).draw();
      }else{
        this.$refs.SessionTable.datatable.clear().rows.add([]).draw();
      }
    })
    .catch(error =>{
      console.log(error);
  ...
  //打開模塊選擇框
  showSessionModal(){
    if((this.currentMaster.rp020 || '') == '') return this.showMessage(this.$t('Please select the system first!')); //請先選擇系統!
    this.$refs.SessionTable.datatable.clear().rows.add([]).draw();
    this.get_sessionTable(this.currentMaster.rp020);
    this.currentTamp = {};
    this.$refs.sessionModal_ref.width("900px");
    this.$refs.sessionModal_ref.show();
  },
  //獲取聯繫人
  getContacts() {
  ...
  generateHeaders(columns) {
    ...
    const fieldDescriptions = [
      ...
      { me002: this.$t('Reporting Department Name') }, //提出部門名稱 Department Name Reporting
      // 继续添加其他字段与描述的映射关系
    ];
  ...
  //刪除流程圖
  deleteFlowChart(){
    axios.post(`/systembugrpt/flowchart/delete`,this.objectToFormData(this.flowChart),{
      headers: {
        "X-CSRFToken": getCookie('csrftoken')
      }
    }).then((response) => {
  ...
  masterSearch(){
    ...
    query.then((query) => {
        ...
        if (query.master != undefined)
            ...
            self.$nextTick(function () {
                self.$refs.AdmrpTable.datatable.column(0).search("").draw();// 先發起搜尋並重繪表格
                // 監聽 draw 完成事件
                self.$refs.AdmrpTable.datatable.one('draw.dt', () => {
                    // 畫面重繪完成後執行後續操作
                    var tableData = self.$refs.AdmrpTable.datatable.data();
                    self.getMasterInfo(tableData[0].inc_id, '');
                });
            });
    });
  }, 
  ...
  arrangeTaskSave(){
    ...
    axios.post(`/systembugrpt/task/create`,this.objectToFormData(this.currentTask),{
      headers: {
        "X-CSRFToken": getCookie('csrftoken')
      }
    }).then(response =>{
  ...
  deleteAdmrf(){
    ...
    let flag = confirm(this.$t("Are you sure you want to delete it"));
    if (flag) {
      axios.post(`/systembugrpt/admrf/delete`,this.objectToFormData(this.currentAdmrf),{
        headers: {
          "X-CSRFToken": getCookie('csrftoken')
        }
      }).then((response) => {
  ...
  admrfSave(){
    if(this.currentAdmrf.inc_id == undefined){ //新增
      axios.post(`/systembugrpt/admrf/create`,this.objectToFormData(this.currentAdmrf),{
        headers: {
          "X-CSRFToken": getCookie('csrftoken')
        }
      }).then(response =>{
  ...
  //上傳多張圖片通用請求方法(多張)
  uploadImgArray(args){
    axios.post(`/systembugrpt/upload_imgarray`,this.objectToFormData(args),{
      headers: {
        "X-CSRFToken": getCookie('csrftoken')
      }
    }).then(response =>{
  ...
  //系統列表單擊事件 
  project_row_click(event,data){
    this.currentTamp.rp020 = data.recordid;
    this.currentTamp.rp020c = data.recordid + '(' + (data.projectname || '') + ')';
  },
  //用戶選擇系統編號
  systemClick(){
    ...
    this.currentMaster.rp020c = this.currentTamp.rp020c;
    this.$refs.projectModal_ref.hide();
  },
  //用戶選擇系統名稱(打開彈框)
  showSystemModal(){
    this.currentTamp = {};
    this.$refs.projectModal_ref.width("900px");
    this.$refs.projectModal_ref.show();
  },
  //取消按鈕
  undoMaster(){
  ...
  deleteMaster(){
    ...
    let flag = confirm(this.$t("Are you sure you want to delete it"));
    if (flag) {
      axios.post(`/systembugrpt/admrp/delete`,this.objectToFormData(this.currentMaster),{
        headers: {
          "X-CSRFToken": getCookie('csrftoken')
        }
      }).then((response) => {
  ...
  async saveMasters(){
    ...
    .then(newToken => {
      ...
      // 每次提交前從 Cookie 中重新讀取最新的 CSRF Token
      if(this.masterState == 1){ //新增
        axios.post(`/systembugrpt/admrp/create?ts=${timestamp}`,form, {
          headers: {
          ...
          if(response.data.inc_id){
            ...
            this.showMessage(this.$t('the new failure')+': '+response.data.msg); //新增失敗
            this.$refs.buttonBar.setToolButtonState(true);
          }
          this.$nextTick(function () {
            $('select.select2-problemcategory').trigger('change');
        ...
        axios.post(`/systembugrpt/admrp/update?ts=${timestamp}`,form, {
          headers: {
            "X-CSRFToken": newToken
          }
        }) //每次發送請求時添加一個隨機數,只要URL有變化,瀏覽器就不會使用舊的緩存數據
        .then((response) => {
          this.isLoading = false; // 停止加载
          ...
          this.showMessage(this.$t("change failed")+': '+response.data.msg); //修改失敗
          this.$refs.buttonBar.setToolButtonState(true);
          this.$nextTick(function () {
            $('select.select2-problemcategory').trigger('change');
          })
    ...
    .catch(error => {
      // 刷新 Token 失败时的处理：例如提示用户重试或刷新页面
      alert("无法刷新安全令牌，请刷新页面或重新登录。");
    });
  },
  saveMaster(){
    if(String(this.currentMaster.rp003 || "") == ""){
    ...
    .then(newToken => {
      ...
      //新增前的校驗未加
      if(this.masterState == 1){ //新增
        axios.post(`/systembugrpt/admrp/create?ts=${timestamp}`,this.objectToFormData(this.currentMaster),{
          headers: {
            "X-CSRFToken": newToken
          }
        }).then(response =>{
        ...
        axios.post(`/systembugrpt/admrp/update?ts=${timestamp}`,this.objectToFormData(this.currentMaster),{
          headers: {
            "X-CSRFToken": newToken
          }
        })
  ...
  refreshCsrfToken() {
    // await axios.get('/systembugrpt/refresh_csrf');  // 需在后端添加此接口
    return axios.get('/systembugrpt/refresh_csrf')
    .then(response => {
      var newToken = response.data.csrfToken;
      // 更新 Cookie 中的 Token（如果需要的话）
      // document.cookie = "csrftoken=" + newToken + "; path=/";
      // console.log("刷新后的 Token:", newToken);
      return newToken;
    })
    .catch(error => {
      console.error("刷新 CSRF Token 失敗:", error);
      // 可根據情況提示用戶或中斷保存流程
      throw error;
    });
  }
