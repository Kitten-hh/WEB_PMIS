...
def get_query_filter(id):
    ...
    '''
    query_filter = models.Queryfilter.objects.values('qf013').get(qf025=id)
...
def get_query_filter_advanced(id):
    query_filter = models.Queryfilter.objects.values('qf013').get(qf025=id)
    str_filter = query_filter['qf013']
    str_filter = convertDate(str_filter)
    str_filter = handlePercent(str_filter)
    str_filter = analyzeQueryFilter(str_filter)
    top = get_top_with_query_filter(id)    
    return str_filter,top


def get_top_with_query_filter(id):
    '''
    功能描述：根據查詢條件判斷是否需要讀取前多少條數據
    '''
    rs = models.Syspara.objects.values('fvalue').filter(ftype='TaskEnquiryTop').extra(tables=['QueryFilter'], where=["QueryFilter.QF003 like '%%' + Syspara.nfield + '%%' and QueryFilter.qf025=%s"], params=[id])
    if len(rs):
        return rs[0]['fvalue']
    else:
        return None
...
def analyzeQueryFilter(queryFilter, doubleSeparate=False):
    ...
    if queryFilter:
        ...
        if re.search("= NULL",queryFilter, re.IGNORECASE) or re.search("=NULL",queryFilter, re.IGNORECASE):
            queryFilter = queryFilter.replace("= NULL", "IS NULL")
            queryFilter = queryFilter.replace("=NULL", "IS NULL")
    return queryFilter
...
def get_daily_goal_filter(str_filter, is_daily=True, order_num=None, owner=None):
    '''
    功能描述：獲取用戶daily 查詢條件
    order_num 查詢條件排序序號
    '''
    ...
    if owner:
        rs = models.VQueryfilter.objects.filter(Q(qf006=owner,qf009='PMS',qf010='TaskEnquiry_Frm') & (qf003_filter | qf008_filter))
    else:
        rs = models.VQueryfilter.objects.filter(Q(qf006='sing',qf009='PMS',qf010='TaskEnquiry_Frm') & (qf003_filter | qf008_filter))
    if is_daily:
        rs = rs.filter(qt002__contains='Daily Goal')
    if order_num:
        rs = rs.filter(qf012=order_num)
    return list(rs);