    ...
    $(function () {
        ...
        var precautiondata = new Array()
        var statusdata = [{value_field:'S', label_field:'S:Satis fied'},{value_field:'A', label_field:'A:Alternative'},
                    {value_field:'N', label_field:'N:UnKnown'},{value_field:'T', label_field:'T:Need to Test'},
                    {value_field:'I', label_field:'I:Checking'},{value_field:'C', label_field:'C:Confirmed'}]
        var contact = new SWCombobox("mb020", "", statusdata,undefined,'value_field','label_field');
        $("#statusdiv").append(contact.dom);
        ...
        form1.on_save_format = function(data){
            if (data.mb005==undefined){
                data.mb005 =$('input[name="mb005"]').val()
            }
        ...
        form1.on_init_format = function(data) {
            ...
            window.setTimeout(function () {
                getTechnicId()
                form1.redirect_url = "/PMIS/opportunity/Technical_Material?detailplanner=Y&param="+$('input[name="mb023"]').val()+'&inc_id='+$('input[name="inc_id"]').val();
            }, 100);
        }

        form1.on_after_init = function (data) {
            var param = data.mb023
            var inc_id = data.inc_id
            if(data.mb027==null){
                data.mb027='0'
            }
            form1.redirect_url = "/PMIS/opportunity/Technical_Material?detailplanner=Y&param="+param+'&inc_id='+inc_id;
            //獲取對應的SolutionType
            $.ajax({
                type: "GET",
                url: "/PMIS/opportunity/Get_technical?param="+param+'&inc_id='+inc_id,
                dataType: "json",
                async: false,
                success: function (result) {
                    if (result.state==200) {
                        stepsdata = result.data['steps']
                        exampledata = result.data['example']
                        precautiondata = result.data['precaution']
                        var state = ''
                        for (var i = 0; i < stepsdata.length; i++) {
                            var strhtml = '<div id="typefirst['+stepsdata[i].mc004+']" name="typefirst['+stepsdata[i].mc004+']" ondblclick="dbOpen(this)" class="toUpdate">'+
                            '<li><pre name="typefirst['+stepsdata[i].mc004+'].pre">'+
                            '</pre></li>'+
                            '<li><img src="/PMIS/opportunity/Technical_Image?id='+
                            stepsdata[i].id+'-'+stepsdata[i].mc003+'-'+stepsdata[i].mc004+
                                    '" onerror="$(this).parent().remove();" alt="" name="typefirst['+stepsdata[i].mc004+'].img" class="mw-100"></li></div>'
                            $("#Stepstab").append(strhtml);
                            $('pre[name="typefirst['+stepsdata[i].mc004+'].pre"]').text(stepsdata[i].mc005) 
                            var strhtml ='<textarea name="typefirst['+stepsdata[i].mc004+'].mc005" cols="30" rows="10" class="col-12 formtextaret"></textarea>'+
                            '<input name="typefirst['+stepsdata[i].mc004+'].imginput" type="text">'+
                            '<input type="checkbox" class="" name="typefirst['+stepsdata[i].mc004+'].deletedata" value="">'+
                            '<input type="checkbox" class="" name="typefirst['+stepsdata[i].mc004+'].deleteimg" value="">'
                            $("#Stepshidden").append(strhtml);
                            $("#Stepshidden textarea[name='typefirst["+stepsdata[i].mc004+"].mc005'").val(stepsdata[i].mc005)
                        }
                        for (var i = 0; i < exampledata.length; i++) {
                            var strhtml = '<div id="typesecond['+exampledata[i].mc004+']" name="typesecond['+exampledata[i].mc004+']" ondblclick="dbOpen(this)" class="toUpdate">'+
                            '<li><pre name="typesecond['+exampledata[i].mc004+'].pre">'+
                            '</pre></li>'+
                            '<li><img src="/PMIS/opportunity/Technical_Image?id='+
                                    exampledata[i].id+'-'+exampledata[i].mc003+'-'+exampledata[i].mc004+
                                    '" onerror="$(this).parent().remove();" alt="" name="typesecond['+exampledata[i].mc004+'].img" class="mw-100"></li></div>'
                            $("#Exampletab").append(strhtml);
                            $('pre[name="typesecond['+exampledata[i].mc004+'].pre"]').text(exampledata[i].mc005) 
                            var strhtml ='<textarea name="typesecond['+exampledata[i].mc004+'].mc005" cols="30" rows="10" class="col-12 formtextaret"></textarea>'+
                            '<input name="typesecond['+exampledata[i].mc004+'].imginput" type="text">'+
                            '<input type="checkbox" class="" name="typesecond['+exampledata[i].mc004+'].deletedata" value="">'+
                            '<input type="checkbox" class="" name="typesecond['+exampledata[i].mc004+'].deleteimg" value="">'
                            $("#Examplehidden").append(strhtml);
                            $("#Examplehidden textarea[name='typesecond["+exampledata[i].mc004+"].mc005'").val(exampledata[i].mc005)
                        }
                        for (var i = 0; i < precautiondata.length; i++) {

                            var strhtml = '<div id="typethird['+precautiondata[i].mc004+']" name="typethird['+precautiondata[i].mc004+']" ondblclick="dbOpen(this)" class="toUpdate">'+
                            '<li><pre name="typethird['+precautiondata[i].mc004+'].pre">'+
                            '</pre></li>'+
                            '<li><img src="/PMIS/opportunity/Technical_Image?id='+
                                    precautiondata[i].id+'-'+precautiondata[i].mc003+'-'+precautiondata[i].mc004+
                                    '" onerror="$(this).parent().remove();" alt="" name="typethird['+precautiondata[i].mc004+'].img" class="mw-100"></li></div>'
                            $("#Precautiontab").append(strhtml);
                            $('pre[name="typethird['+precautiondata[i].mc004+'].pre"]').text(precautiondata[i].mc005) 
                            var strhtml ='<textarea name="typethird['+precautiondata[i].mc004+'].mc005" cols="30" rows="10" class="col-12 formtextaret"></textarea>'+
                            '<input name="typethird['+precautiondata[i].mc004+'].imginput" type="text">'+
                            '<input type="checkbox" class="" name="typethird['+precautiondata[i].mc004+'].deletedata" value="">'+
                            '<input type="checkbox" class="" name="typethird['+precautiondata[i].mc004+'].deleteimg" value="">'
                            $("#Precautionhidden").append(strhtml);
                            $("#Precautionhidden textarea[name='typethird["+precautiondata[i].mc004+"].mc005'").val(precautiondata[i].mc005)
                        }
                        if((result.data['mb015ctext'] !='') && (result.data['mb015ctext'] != null)){
                            $('input[name="mb015c"]').val(result.data['mb015ctext'])
                        }
                    }
                }
            })
        ...
        //     getTechnicId();
        //     form1.redirect_url = "/PMIS/opportunity/Technical_Material?param="+$('input[name="mb023"]').val()+'&inc_id='+$('input[name="inc_id"]').val();
        // }
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                for (var i = 0; i < arrays.length; i++) {
                    var type = '#'
                    if(arrays[i].ma002.replaceAll(' ','') != '' && arrays[i].ma002.replaceAll(' ','')==arrays[i].ma007.replaceAll(' ',''))
                        type='level1'
                    if(arrays[i].ma002.replaceAll(' ','') != '' && arrays[i].ma002.replaceAll(' ','')!=arrays[i].ma007.replaceAll(' ',''))
                        type='level2'
                    var arr = {
                        "id": arrays[i].ma001,
                        "parent": arrays[i].ma002.replaceAll(' ','') == '' ? "#" : arrays[i].ma002.replaceAll(' ',''),
                        "text": arrays[i].ma003,
                        'data':arrays[i],
                        'type':type
                    }
    ...
    function openmodal(strname){
        $("#myModal textarea[name='modaltextarea']").val($("textarea[name='"+strname+".mc005']").val());
        $("#myModal input[name='modalimginput']").val($("input[name='"+strname+".imginput']").val())
    ...
    function dbOpen(e){
        ...
        var strhtml ='<div id="myModal"><div class="modal-body">'+
                        ...
                        '<input class="getFile" name="modalimginput" type="file" onchange="imgChange(this)"  accept="image/jpg">上傳圖片</span>'+
                        '<img name="modalimg" class="mw-100"></div><div class="modal-footer">'+
                    '<button type="button" class="btn btn-primary" onclick="Deletedataclick()" id="Deletedatabtn">刪除本條數據</button>'+
                    '<button type="button" class="btn btn-primary" onclick="Deleteimgclick()" id="Deleteimgbtn">刪除圖片</button>'+
                    '<button type="button" class="btn btn-primary" onclick="Enterclick()" id="Enterbtn">确定</button>'+
                    '<button type="button" class="btn btn-light technicmodelhidden" onclick="modalhidden()" id="modalhiddenbtn">取消</button>'+
                    '</div></div>'
        var strname = e.id
    ...
    function addhtml(divId,typeindex,hiddivid){
        ...
        var strname = $('#'+divId).children().last()[0]
        if(strname==undefined){
            strname=1
        }else{
            strname = strname.id
            strname = parseInt(strname.substring(strname.indexOf('[')+1,strname.indexOf(']')))+1
        }
