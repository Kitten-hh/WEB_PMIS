<template>
    <div style="width: 100%; display: flex; justify-content: space-between">
        <div id="myDiagramDiv" style="flex-grow: 1; height: 870px;background-color:white"></div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        init() {
            ...
            const $m = go.GraphObject.make;

            self.myDiagram =
                $m(go.Diagram, "myDiagramDiv",  // must name or refer to the DIV HTML element
                    {
            ...
            self.myDiagram.toolManager.draggingTool.doActivate = function () {  // method override must be function, not =>
                go.DraggingTool.prototype.doActivate.call(this);
                this.currentPart.opacity = 0.6;
                this.currentPart.layerName = "Foreground";
            }
            self.myDiagram.toolManager.draggingTool.doDeactivate = function () {  // method override must be function, not =>
            ...
            function relayoutDiagram() {
                self.myDiagram.selection.each(n => n.invalidateLayout());
            ...
            self.myDiagram.nodeTemplate =
                $m(go.Node, "Horizontal",
                    ...
                    $m(go.Shape, "Rectangle", {
                        ...
                        click: (e, obj) => {
                            self.myDiagram.startTransaction("Update node color");
                            let newColor = parseInt(obj.part.data.color) + 1;
                            if (newColor > noteColors.length - 1) newColor = 0;
                            self.myDiagram.model.setDataProperty(obj.part.data, "color", newColor);
                            self.myDiagram.commitTransaction("Update node color");
                        }
                    },
                    ...
                    $m(go.Panel, "Auto",
                        $m(go.Shape, "Rectangle", { fill: "white", stroke: '#CCCCCC' }),
                        $m(go.Panel, "Table",
                            { width: 130, 
            ...
            function highlightGroup(grp, show) {
                if (show) {
                    const part = self.myDiagram.toolManager.draggingTool.currentPart;
                ...
                $m(go.Group, "Vertical",
                    {
                        ...
                        layout: $m(go.GridLayout,  // automatically lay out the lane's subgraph
                            {
                                wrappingColumn: 1,
                    ...
                    $m(go.Panel, "Horizontal",
                        { name: "HEADER", alignment: go.Spot.Left },
                        $m("SubGraphExpanderButton", { margin: 5 }),  // this remains always visible
                        $m(go.TextBlock,  // the lane label
                    ...
                    $m(go.Panel, "Auto",  // the lane consisting of a background Shape and a Placeholder representing the subgraph
                        $m(go.Shape, "Rectangle",  // this is the resized object
                        ...
                        $m(go.TextBlock,  // this TextBlock is only seen when the swimlane is collapsed
                            {
                    ...
                    $m(go.Panel, "Horizontal",
                        {
                            ...
                            click: (e, node) => {
                                ...
                                const newnode = self.myDiagram.findNodeForData(newdata);
                        ...
                        $m(go.Panel, "Auto",
                            $m(go.Shape, "Rectangle", { strokeWidth: 0, stroke: null, fill: '#6FB583' }),
                            $m(go.Shape, "PlusLine", { margin: 6, strokeWidth: 2, width: 12, height: 12, stroke: 'white', background: '#6FB583' })
                        ),
                        $m(go.TextBlock, "New item", { font: '10px Lato, sans-serif', margin: 6, })
                    )
                )
            );
            this.load();
        ...
        load(value) {
            if (value)
                self.myDiagram.model = go.Model.fromJson(value);
            else
                self.myDiagram.model = go.Model.fromJson({ "class": "go.GraphLinksModel",
