...
var GanttView = /*#__PURE__*/function () {
  ...
  _createClass(GanttView, [{
    ...
    value: function getTasks() {
      ...
      var task = []
      var params = {}
      if ($("#calendar_param_type").length != 0 && $("#calendar_param_type").val() != '') {
        var type_str = $("#calendar_param_type").val();
        var id = $("#calendar_param_id").val();
        params['type'] = type_str
        params['id'] = id
        if (type_str == 'goal' && $("#calendar_param_show_level").length >= 0)
          params['show_level'] = $("#calendar_param_show_level").val();
      }

      $.get("looper_gantt_data", params, function(data){
        task = data
      });
