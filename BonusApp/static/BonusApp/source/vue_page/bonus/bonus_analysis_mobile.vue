<template>
    <div id="app" class="analysis_page page-inner">
        <div class="subheader">
            <div class="dropdown" id="task_search_box">                    
                <div class="el-example task_search_tools">   
                    <button class="btn btn-fixed-height btn-primary font-weight-bolder" @click="search('Bonus')"><i class="fa fa-search mr-2"></i>{{$t("Search Bonus")}}</button>
                    <button class="btn btn-fixed-height btn-secondary btn-hover-text-primary font-weight-bolder"
                        @click="bonusAnalysis('yesterday')">{{$t("Yesterday")}}
                    </button>
                    <button class="btn btn-fixed-height btn-secondary btn-hover-text-primary font-weight-bolder"
                        @click="bonusAnalysis('today')">{{$t("Today")}}
                    </button>  
                    <button class="btn btn-fixed-height btn-secondary btn-hover-text-primary font-weight-bolder"
                        @click="bonusAnalysis('lastweek')">{{$t("Last Week")}}
                    </button> 
                    <button class="btn btn-fixed-height btn-secondary btn-hover-text-primary font-weight-bolder"
                        @click="bonusAnalysis('week')">{{$t("This Week")}}
                    </button>                      
        ...
        <div style="margin-top: 12px;">   
            <div class="row">                
                <div class="col-12 col-lg-6 col-xl-7 col-xxl-9">
                    ...
                    <div id="search_card_accordion" class="card-expansion mb-2">
                        <div id="query_wrapper" class="card card-custom gutter-b task_card bg-gray-100 card-expansion-item expanded" data-card="true">
                            <div class="card-header border-0 d-flex flex-wrap">
                                <button
                                    class="btn btn-reset d-flex justify-content-between"
                                    data-toggle="collapse"
                                    data-target="#search_card_collapse"
                                    aria-expanded="true"
                                    aria-controls="search_card_collapse"
                                >
                                    <span class="collapse-indicator mr-3">
                                ...
                                <div class="card-toolbar ml-auto ml-sm-0 ml-lg-auto">                               
                                    <button id="btn_auditScore" class="btn btn-fixed-height btn-primary font-weight-bolder font-size-sm px-2 mr-2" style="display:none" @click="auditScore('Y')">{{$t("Post")}}
                                    </button>  
                                    <button id="btn_unauditScore" class="btn btn-fixed-height btn-primary font-weight-bolder font-size-sm px-2" style="display:none" @click="auditScore('R')">{{$t("UnPost")}}
                                    </button>
                                </div>
                            </div>       
                                ...
                                <div class="tab-content pt-1">
                                    ...
                                    >
                                        <div class="card card-reflow mb-0">
                                            <div class="card-title dropdown m-0">                    
                                                <div class="form-inline task_search_tools mt-2">   
                                                    <button class="btn btn-fixed-height btn-primary font-weight-bolder mr-2 ml-2" @click="search">
                                                        <i class="fa fa-search"></i>
                                                    </button>   
                                                    <button id="btnNew" type="button" class="btn btn-primary" @click="newDeductionItem">
        ...
        <div id="search_tab" class="d-none">
            <ul class="nav nav-tabs nav-tabs-line mb-5 fs-6 ml-2" style="font-size:14px;">
                <li class="nav-item">
                    <a class="nav-link show active" data-toggle="tab" href="#search_basic">{{$t("General Query")}}</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" data-toggle="tab" href="#search_advanced">{{$t("Advanced Query")}}</a>
                </li>
            </ul>
    ...
    <LPModal ref="UserModal" :title="$t('Please enter the user name')" class="col" id="UserModal">
      <template v-slot:body>
        <!-- :gsearch_placeholder="a" -->
  ...
  data() {
    ...
    return {        
        taskColumns: [
            ...
            { field: "task", label: this.$t("Description"), width: "270px" , className: 'min-tablet'},
            //{field:"edate", label:"E Date", render:SWDataTable.DateRender},            
            {
                field: "edate", label: this.$t("E Date"), width:"80px",
                className: 'desktop',
                ...
                render:function(data, type, row){    
                    ...
                    <button class="btn caption" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-ellipsis-v"></i></button>  
                        <div class="dropdown-menu control" aria-labelledby="dropdownMenuButton">
                            <a class="dropdown-item edit" href="#" style="color: #191927">{0}</a>
                            <a class="dropdown-item delete" href="#" style="color: #191927">{1}</a>
                        </div>
  ...
  methods: {  
    ...
    bonusAnalysis_handle(){
        ...
        axios.get("/bonus/bonus_analysis",{
            params: { "contact": this.contact, "edatefrom": this.global_edatefrom, "edateto": this.global_edateto , "calSat":false, "quarterbegin":quarterBegin, "quarterend":quarterEnd}
        })
        .then((response) => {
            $("#tab_tasks").click();
    ...
    showTasks(data){        
        ...
        this.$refs.dt_task.datatable.clear();
        this.$refs.dt_task.datatable.rows
        .add(data).draw();
        this.getBrowser();
    },     
        ...
        $("#dt_userdeduction").on("click", ".edit", function(e){
            ...
            .then((response)=>{
                ...
                if(result.status){
                    ...
                    self.deductionItemFormTitle = self.$t("Eidt Item");
