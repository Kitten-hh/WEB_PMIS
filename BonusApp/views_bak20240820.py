...
from DataBase_MPMS.models import Tpdetail, VTask,VTaskbonussl, VUsers, Tasktype, Task, VTasktypetreelist, VQueryfilter, Tecmb, Tasktypelist,Syspara,TasktypeSl,TasktypeSlHistory, Deductionitem,Userdeduction,LSTasktypelist,Improvearea
...
def calculate_bonus(bonusPara):
    ...
    if not bonusPara["PeriodWorkingDay"]==0:
        ScoreAvg = (totalPeriodScore + totalPeriodManagementScore*bonusPara["ManagementRatio"]/100 + totalPeriodScore*bonusPara["PerformanaceRatio"]/100) / bonusPara["PeriodWorkingDay"]
    else:
        ScoreAvg = 0
    # 每日平均Lookup Score
    ...
    if not bonusPara["WorkingDay"]==0:
        QuarterScore = totalScore + (totalManagementScore*bonusPara["ManagementRatio"]/100 + totalScore*bonusPara["PerformanaceRatio"]/100) / bonusPara["WorkingDay"]
        QuarterLookupScore = totalLookupScore + (totalManagementLookupScore*bonusPara["ManagementRatio"]/100 + totalLookupScore*bonusPara["PerformanaceRatio"]/100) / bonusPara["WorkingDay"]
    else:
        QuarterScore = 0
