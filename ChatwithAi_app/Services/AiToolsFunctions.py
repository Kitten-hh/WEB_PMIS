...
def filter_empty_columns(data, headers):
    ...
    headers = list(headers)  # 确保 headers 是列表
    # 找出非空字段索引,强制排除 inc_id（无论大小写）
    valid_columns = [
        col_idx for col_idx in range(len(headers))
        if headers[col_idx].lower() != "inc_id" and any(row[col_idx] is not None for row in data)
    ]
    # 过滤字段
...
def tabulate_with_chinese(data, headers):
    ...
    def get_col_widths(data, headers):
        ...
        for col_idx in range(len(headers)):
            col_values = [
                line for row in data + [headers]  # 包括表头和数据
                if row[col_idx] is not None       # 排除 None
                for line in str(row[col_idx]).splitlines()  # 分行处理
            ]
            # 如果列为空序列，提供默认宽度
            max_width = max((wcswidth(line) for line in col_values), default=1)
        ...
        if result.get('status'):
            ...
            for item in data:
                ...
                if message_text == "":
                    ...
                    message_text += "\r\n" + line_text
            display_text = format_result(data)
            informaction = None
            if action_name:
                informaction = globals()[action_name](data)
            return message_text,display_text, informaction
        return "","", ""
