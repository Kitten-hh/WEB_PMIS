...
import os
from ..models import TopicCategories, MainQuestions, SubQuestions,TopicRelationTables
...
class ConversationService:
    ...
    def load_topic_info(self, user_iniput, userid, session, category):
        ...
        if len(qs) > 0:
            ...
            self.load_topic_data(topic, session)
    
    def load_topic_data(self, topic, session):
        # Pass tables and views to load_management_template_data
        tables = [relation for relation in TopicRelationTables.objects.filter(topic_id=topic.id) if relation.table_name]
        views = [relation for relation in TopicRelationTables.objects.filter(topic_id=topic.id) if relation.view]
        try:
            datas = self.load_management_template_data(tables, views)
            # Convert each array of data to JSON and add to session
            for data_array in datas:
                try:
                    message = json.dumps(data_array, cls=DjangoJSONEncoder)
                    session['context'].append({"role": "assistant", "content": message})
                except (TypeError, ValueError) as e:
                    LOGGER.error(f"Failed to convert data to JSON: {e}")        
        except Exception as ex:
            LOGGER.error(e)

    def load_management_template_data(self, tables, views):
        # Read AI parameters from Syspara
        time_range = Syspara.objects.get(nfield='AI_DataTimeRange').fvalue
        time_fields = Syspara.objects.get(nfield='AI_DataTimeField').fvalue.split(';')
        max_data_len = int(Syspara.objects.get(nfield='AI_MaxData_GPT4').fvalue)
        limit_record = int(Syspara.objects.get(nfield='AI_DataLimitRecord').fvalue)

        results = []
        for tbl in tables:
            table_name = tbl.table_name
            limit_type = tbl.limit_type
            try:
                ...
                # Log the error and skip to the next table
                LOGGER.error(f"Failed to retrieve columns for table {table_name}: {e}")
                continue

            # Find the correct date field
            date_field = next((field for field in time_fields if field in columns), None)
            if date_field:
                if limit_type == 'D':
                    # Query without date restriction, limit by record count
                    query = f"""
                        SELECT TOP {limit_record} * FROM {table_name}
                        ORDER BY {date_field} DESC
                    """
                else:
                    # Query with date restriction
                    query = f"""
                        SELECT TOP {max_data_len} * FROM {table_name}
                        WHERE {date_field} BETWEEN CONVERT(VARCHAR(8), DATEADD(DAY, -{time_range} + 1, GETDATE()), 112)
                        AND CONVERT(VARCHAR(8), DATEADD(DAY, 1, GETDATE()), 112)
                        ORDER BY {date_field} DESC
                    """
                ...
                with connections['MPMS'].cursor() as cursor:
                    ...
                    table_results = [dict(zip(columns, row)) for row in rows]
                    results.append(table_results)

        for vw in views:
            view_sql = vw.view.sql_statement
            limit_type = vw.limit_type
            try:
                # Retrieve columns of the view
                with connections['MPMS'].cursor() as cursor:
                    cursor.execute(f'Select * from ({view_sql}) As subquery WHERE 1=0')
                    columns = [col[0] for col in cursor.description]
            except Exception as e:
                # Log the error and skip to the next view
                LOGGER.error(f"Failed to retrieve columns for view: {e}")
                continue
            ...
            if date_field:
                if limit_type == 'D':
                    # Query without date restriction, limit by record count
                    query = f"""
                        SELECT TOP {limit_record} * FROM ({view_sql}) AS subquery
                        ORDER BY {date_field} DESC
                    """
                else:
                    # Query with date restriction
                    query = f"""
                        SELECT TOP {max_data_len} * FROM ({view_sql}) AS subquery
                        WHERE {date_field} BETWEEN CONVERT(VARCHAR(8), DATEADD(DAY, -{time_range} + 1, GETDATE()), 112)
                        AND CONVERT(VARCHAR(8), DATEADD(DAY, 1, GETDATE()), 112)
                        ORDER BY {date_field} DESC
                    """
                ...
                with connections['MPMS'].cursor() as cursor:
                    ...
                    # Convert results to a list of dictionaries
                    view_results = [dict(zip(columns, row)) for row in rows]
                    results.append(view_results)

        # 應用層裁剪數據
        """
        if len(results) > max_data_len:
            results = results[:max_data_len]"""
    ...
    def handle_conversation_status(self, user_id, user_input, session):
        """
        功能描述：更新會話狀態
        """
        if session['current_category_index'] is not None and session['current_question_index'] < len(self.categories[session['current_category_index']]['questions']):  #有主題且主問題沒有問完
            category = self.categories[session['current_category_index']]
            questions = category['questions']

            if user_input and user_input.lower() in ['break','resume']:
                if user_input.lower() == 'break':
                    session['conversation_status'] = 'break'
                    return f"Analysis: We’ve paused the conversation. Go ahead and ask AI anything!\nTo resume the conversation, please input 'resume'."            
                else: #恢復之前的問題
                    session['conversation_status'] = None
                    ...
                    if session['in_sub_question']:
                        if session['current_sub_question_index'] < len(sub_questions):
                            ...
                            return f"Analysis: Conversation resumed. Let's continue where we left off!\nNext sub-question: {sub_question}"
                        else:
                            return f"Analysis: Conversation resumed. Let's continue where we left off!\nNext sub-question:"
                    else: #在主問題中
                        question = questions[session['current_question_index']]
                        return f"Analysis: Conversation resumed. Let's continue where we left off!\nNext question: {question['question']}"            
            elif session['conversation_status'] == 'break': #如果用戶輸入的不是break, resume且當前的狀態是break，則使用chatgpt回答問題
                analysis = self.get_chatgpt_response(user_input, user_id=user_id, session=session)
    ...
    def sub_handle_conversation(self, user_id, user_input, session):
        ...
        status_message = self.handle_conversation_status(user_id, user_input, session)
        if status_message:
