...
import json
import sys
from .models import Topic,MindMapData
...
class Mindmap:
    ...
    def get_mindmap_data_text(self, topic, number):
        ...
        try:
            ...
            #with cursor:
            with connection.cursor() as cursor:
                # Replace 'selected_topic' with your specific topic of interest
                '''
                sql = "SELECT * FROM jsondata WHERE topic = %s"
                cursor.execute(sql, ('Big data',))
                rows = cursor.fetchall()
                '''
                qs = MindMapData.objects.filter(topic=topic).values()
...
def calculate_square(topic, number):
    ##return number * number
    mindmapObj = Mindmap()
    return mindmapObj.get_mindmap_data_text(topic, number)